...
effect increases by $\approx 0.88$ and $\approx 0.86$ units, respectively.
...
1.98$ in the average treatment effect, respectively.
...
1.06$ and $\approx 3.00$ units, respectively. For the TML estimator, the log
ration of the conditional expected outcomes is expected to decrease by $\approx
1.39$ and $\approx 3.52$ for every unit increase in `w_2` and `w_3`,
respectively.
...
@boileau2023 Propose an absolute TEM-VIP based on the restricted mean survival
...
$\approx 1.66$ --- according to the one-step estimator --- or $\approx 1.25$ ---
...
either $\approx 3.29$ or $\approx 3.28$, depending on whether one uses the
...
Learner uses the log loss to form the convex combination of individual learning
algorithms, while the conditional expected outcomes' uses the squared error
loss. Additionally, we employ 2-fold cross-fitting.
...
tests are to those of the previous continuous outcome, absolute TEM-VIP example.
